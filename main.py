...
def checkBirthday():
  for id in birthdays:
    ...
    today = [datetime.now(tz=timezone(timedelta(hours=int(zone)))).month, datetime.now(tz=timezone(timedelta(hours=int(zone)))).day]
    if birthdays[id][4:] == today:
      if birthdays[id][3] == False: 
        bot.send_message(birthdays[id][0], f"Today is the birthday of {birthdays[id][2]}")
      birthdays[id][3] = True
    else:
...
def addBirthday(message):
  ...
  bot.send_message(message.chat.id, "Hello! Let's register a new person's birthday! Enter their name:")
  bot.register_next_step_handler(message, getName)

def getName(message):
  global name
  name = message.text
  bot.send_message(message.chat.id, "Now, what's the month of this peron's birthday? Enter number from 1 (January) to 12 (December)")
...
def getMonth(message):
  ...
  if monthChecker(month) == False:
    bot.reply_to(message, "Error! Enter valid number from 1 (January) to 12 (December)")
    bot.register_next_step_handler(message, getMonth)
  else:
    bot.send_message(message.chat.id, "Now, enter a valid day (Ensure that the day didn't go beyond what is expected in certain month (e.g. there is no 31 in September). Otherwise, I will crash ;)")
...
def getDay(message):
  ...
  if dayChecker(day) == False:
    bot.reply_to(message, "Error! Enter valid number from 1 to 31 with caution!")
    bot.register_next_step_handler(message, getDay)
  else:
    bot.send_message(message.chat.id, "Now, enter valid Time Zone that this person has currently (e.g. if he lives in Aktobe, which is UTC +5, then just type 5)")
...
def getTimezone(message):
  ...
  if zoneChecker(zone) == False:
    bot.reply_to(message, "Error! Enter valid Time Zone (number) from -12 to 14")
    bot.register_next_step_handler(message, getTimezone)
  else:
    birthdays[id] = []
    birthdays[id].extend([message.chat.id, zone, name, False, int(month), int(day)])
    bot.send_message(message.chat.id, f"Well Done! {name} is recordered. I can't forget, I will remind you, trust) To add another person, type /Add command")
